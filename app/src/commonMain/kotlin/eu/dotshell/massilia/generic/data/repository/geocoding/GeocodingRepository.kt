package eu.dotshell.massilia.generic.data.repository.geocoding

import eu.dotshell.massilia.generic.data.models.search.AddressSearchResult
import eu.dotshell.massilia.generic.data.network.geocoding.PhotonFeature
import eu.dotshell.massilia.generic.data.network.geocoding.PhotonGeocodingClient
...
private const val MARSEILLE_LAT = 43.2965
private const val MARSEILLE_LON = 5.3698
private const val MAX_DISTANCE_FROM_MARSEILLE_KM = 80.0
...
internal fun photonFeatureToResult(feature: PhotonFeature): AddressSearchResult? {
    ...
    if (!isNearMarseille(lat, lon)) return null
 ...
 */
internal fun isNearMarseille(lat: Double, lon: Double): Boolean {
    val dLat = (lat - MARSEILLE_LAT) * KM_PER_DEGREE_LAT
    val dLon = (lon - MARSEILLE_LON) * KM_PER_DEGREE_LAT * cos(MARSEILLE_LAT * PI / 180.0)
    return sqrt(dLat * dLat + dLon * dLon) <= MAX_DISTANCE_FROM_MARSEILLE_KM
}
